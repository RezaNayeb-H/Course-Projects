...
knee_image = abs(knee_image);   % for removing the phase likely created by noise(because the picture should only be real)
...
raw_ifft = ifft2(dat);
noisy_image = fftshift(raw_ifft);
noisy_image = abs(noisy_image);   
% for removing the phase likely created by noise(because the picture should only be real)
%----- new code: -----
