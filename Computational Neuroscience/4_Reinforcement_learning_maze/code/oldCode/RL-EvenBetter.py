...
class Agent:
    ...
    """Visualizes the best policy at each cell using arrows. Arrows on the optimal path are shown in green, and others in red."""
    fig, ax = plt.subplots(figsize=(10, 10))
    ax.imshow(1 - self.env.maze, cmap="gray")  # Show maze (inverted for better visibility)
        ...
        for x in range(self.env.nx):
            if self.env.maze[y, x] == 0:  # Ignore walls
                ...
                if valid_actions:  # If there are valid actions
                    ...
                    # Check if the current cell is on the optimal path
                    if (x, y) in optimal_path:
                        arrow_color = 'green'  # Green for optimal path
                    else:
                        arrow_color = 'red'  # Red for other cells

                    # Draw the arrow
                    ax.arrow(x, y, dx * 0.3, dy * 0.3, head_width=0.2, head_length=0.2, fc=arrow_color, ec=arrow_color)
